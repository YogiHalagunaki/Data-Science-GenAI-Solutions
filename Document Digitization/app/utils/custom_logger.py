...
def get_logger(module: str, file_handler=False):
    ...
    # logging.getLogger("azure").setLevel(logging.CRITICAL)  # All azure clients are logged only for critical        # level set as INFO and above
    logger = logging.getLogger()                            # logger object

    logger.setLevel(os.getenv("log_level", "INFO")) 
    
    logger.error = partial(logger.error, exc_info=True)     # get traceback stack of the error

    handler_console = logging.StreamHandler()                       # handler for showing logs on console
    handler_console.setFormatter(CustomFormatter(module=module))    # set CustomFormatter to the handler
    logger.addHandler(handler_console)

    if file_handler:
        handler_file = logging.FileHandler('log_file.log')              # handler for writing logs in file
        handler_file.setFormatter(CustomFormatter(module=module))       # set CustomFormatter to the handler
